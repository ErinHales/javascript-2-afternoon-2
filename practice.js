...
function divider(numbers) {
  ...
  for(var i=0; i<numbers.length; i++) {
    if(numbers[i]%2 === 0) {
      evensOdds[0].push(numbers[i]);
    } else {
      evensOdds[1].push(numbers[i]);
    }
  }
  return evensOdds;
}
...
function addTen(numbers) {
  var realNumbers = [];
  for(var i=0; i<numbers.length; i++) {
    realNumbers.push(Number(numbers[i]));
  }
  var newArr = realNumbers.map(x => x + 10)
  return newArr;
}
...
fireCahlan(devMountainEmployees);
...
function fillUsersArr(myName, myEmail, myPassword, myUsername) {
  var obj = {
    name: myName,
    email: myEmail,
    password: myPassword,
...
fillUsersArr(user1.name, user1.email, user1.password, user1.username);
fillUsersArr("Erin", "mcb@ride", "12345", "E-rizzle");
fillUsersArr("Kyle", "kyle@hales", "ABCDE", "K-dog");
